...
public class FSServiceInnerTest extends FSTestCase {
	...
	public void testIsValidRelpath() throws Exception {
		...
		String[] valids = { "/", "foo", "foo.bar", "rANdoM.xls", "fold/er.txt",
				...
				"a", "ac (also cool).file", "...", "foo....bar",
				"foo/bar../abz" };
		for (String valid : valids) {
			Assert.assertTrue("valid name: " + valid, fss.isValidRelpath(valid));
		}
		Assert.assertFalse("up and out",
				fss.isValidRelpath("foo/../../bar.xls"));
		...
		for (String value : s) {
			Assert.assertFalse("Invalid character " + value,
					fss.isValidRelpath("~"));
		}
	...
	public void testSetRootPath() throws Exception {
		ProjectDir oldrootpath = fss.getRootPath();

		try {
			fss.setRootPath(new ProjectDir("/root/file/that/doesnt/exist"));
			Assert.fail();
		} catch (FileNotFoundException e) {
		}
		{
			File f = new File(mytempdir, "/test.out");
			f.createNewFile();
			try {
				fss.setRootPath(new ProjectDir(f));
				Assert.fail();
			} catch (NotADirectoryException e) {
			}
		}
		new File(mytempdir, "bla").mkdirs();
		String[] dirnames = { mytempdir + "/bla/..", mytempdir + "/!",
				mytempdir + "/#" };
		for (String dirname : dirnames) {
			File f = new File(dirname);
			f.mkdirs();
			Assert.assertTrue("Creating dir successful",
					f.exists() && f.isDirectory());
			try {
				fss.setRootPath(new ProjectDir(dirname));
			} catch (NotADirectoryException e) {
			}
		}
	...
	public void testGetTempFile() throws Exception {
		...
		Assert.assertTrue("in Temp dir",
				filename.startsWith(fss.getTempDir() + File.separator));
	...
	public void setUp() throws Exception {
		...
		if ((new DesktopSupportedChecker()).satisfy()) {
			...
			Assert.assertFalse(fss.getRootPath().getAbsolutePath()
					.startsWith("/home"));
		}
	}
